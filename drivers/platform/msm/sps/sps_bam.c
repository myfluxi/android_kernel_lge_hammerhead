...
polling:
	...
	if ((dev->state & BAM_STATE_MTI) == 0) {
		...
		SPS_DBG1("sps:bam=%xa;source=0x%x;mask=0x%x.\n",
				BAM_ID(dev), source, mask);

		if ((source & (1UL << 31)) && (dev->props.callback)) {
			SPS_DBG1("sps:bam=%xa;callback for case %d.\n",
				BAM_ID(dev), cb_case);
		...
		SPS_DBG1("sps:MTI:bam=%ua;source=0x%x.\n",
				BAM_ID(dev), source);
	}
	...
	if (dev->props.options & SPS_BAM_RES_CONFIRM) {
		...
		SPS_DBG1(
			"sps:check if there is any new IRQ coming:bam=%xa;source=0x%x;mask=0x%x.\n",
				BAM_ID(dev), source, mask);
...
static irqreturn_t bam_isr(int irq, void *ctxt)
{
	...
	SPS_DBG1("sps:bam_isr: bam:%xa; IRQ #:%d.\n",
		BAM_ID(dev), irq);
	...
	if (dev->props.options & SPS_BAM_RES_CONFIRM) {
		if (dev->props.callback) {
			...
			if (ready) {
				SPS_DBG1(
					"sps:bam_isr: handle IRQ for bam:%xa IRQ #:%d.\n",
					BAM_ID(dev), irq);
				...
				SPS_DBG1(
					"sps:bam_isr: BAM is not ready and thus skip IRQ for bam:%xa IRQ #:%d.\n",
					BAM_ID(dev), irq);
			}
		} else {
			SPS_ERR(
				"sps:Client of BAM %xa requires confirmation but does not register callback\n",
				BAM_ID(dev));
		}
	} else {
		sps_bam_check_irq(dev);
	...
	if ((dev->props.options & SPS_BAM_OPT_IRQ_DISABLED) != 0 ||
		...
		if (dev->props.irq > 0) {
			if (dev->props.options & SPS_BAM_RES_CONFIRM) {
				...
				SPS_DBG(
					"sps:BAM %xa uses edge for IRQ# %d\n",
				...
				SPS_DBG(
					"sps:BAM %xa uses level for IRQ# %d\n",
					BAM_ID(dev), dev->props.irq);
			}
		} else {
			SPS_DBG1("sps:BAM %xa does not have an vaild IRQ# %d\n",
				BAM_ID(dev), dev->props.irq);
		}
